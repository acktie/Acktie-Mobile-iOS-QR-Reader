...
protected:
	// Helpers
	// 2012-05-23 hfn method contents of init ... helpers moved here.
...
template <class X>
X convertToPrimitive(const BigUnsigned&bu) {
	NumberlikeArray<BigUnsigned::Blk>::Index len = bu.getLength();
	if (len == 0)
		// The number is zero; return zero.
		return 0;
	else if (len == 1) {
		BigUnsigned::Blk blk0 = bu.getBlock(0);
		// The single block might fit in an X.  Try the conversion.
		X x = X(blk0);
		// Make sure the result accurately represents the block.
		if (BigUnsigned::Blk(x) == blk0)
...
template <class X>
X convertToSignedPrimitiveUI(const BigUnsigned &bu) {
	X x = convertToPrimitive<X>(bu);
	if (x >= 0)
		return x;
	else
		throw "BigUnsigned::to(Primitive): "
